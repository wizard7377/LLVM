...
public export 
record WithScope {default AnyPtr ref : Type} (me : Type) where 
  constructor MkWithScope
  scope : SortedMap String ref
  value : me
...
record FCState where
  ...
  cBuilders : List (WithScope LLVMBuilder)
public export 
FCM : Type -> Type 
FCM = EitherT FCError (RWST Int () FCState IO)

public export
noScope :  {me : Type} -> {0 ref : Type} -> me -> WithScope {ref = ref} me
