...
load :
    ...
    {default Nothing align : Maybe Nat} ->
    LExpr
load {volatile} ty ptr {align} = (Load volatile ty (toRuntime ptr) align False False False False Nothing Nothing Nothing False)
...
call : 
    ...
    {default [] fnBundles : List OperandBundle} ->
    LExpr
call {tail} {fastMath} {cc} {returnAttrs} {addressSpace} tpe fnval args {fnAttrs} {fnBundles} =
    FnCallOp tail fastMath cc returnAttrs addressSpace tpe (toRuntime fnval) (toRuntime' <$> args) fnAttrs fnBundles
...
mutual
    ...
    simpleCall : 
        ...
        (args : List (WithType (LValue _))) ->
        LExpr
    simpleCall tpe fnval args = simpleCall tpe fnval args
...
export 
getElementPtr : 
  {default NoWrap wrap : Wrapping} -> {default GEPAll range : GEPRange} -> LType -> LValue False -> List (WithType (LValue True)) -> LExpr
getElementPtr {wrap,range} ty v idx = GetElemenentPtr wrap range ty v idx
